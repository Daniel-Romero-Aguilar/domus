    ...
    <aside id="sidebar" class="sidebar">
        ...
        <div id="sidebar-scroll" class="sidebar-scroll">
            <nav class="sidebar-nav">
                ...
                <a class="sidebar-link is-active" href="/account/education"><span class="nav-icon">•</span><span>Educacion</span></a>
            </nav>
...
function renderPart(part){if(part.type==='title'){return '<h3>'+part.content+'</h3>';} if(part.type==='string'){return '<p>'+part.content+'</p>';} if(part.type==='video_youtube'){const embed=youtubeEmbed(part.content);return embed?'<div class="video-wrap"><iframe width="100%" height="220" src="'+embed+'" title="Video" frameborder="0" allowfullscreen></iframe></div>':'<p>Video invalido</p>';} if(part.type==='image_url'){return '<img class="edu-image" src="'+part.content+'" alt="Leccion imagen">';} return '<p>'+part.content+'</p>';}
async function loadEducation(){if(!getToken()){window.location.href='/login';return;}try{const me=await apiRequest('/me','GET');sidebarUserName.textContent=me.user?.name||'Usuario';if(tasksLink){tasksLink.href=(me.user?.role==='parent'?'/parent/tasks':'/child/tasks');}if(loansLink){loansLink.href=(me.user?.role==='parent'?'/parent/loans':'/member/loans');}const data=await apiRequest('/education/courses','GET');const courses=data.courses||[];if(courses.length===0){educationContent.innerHTML='<section class="quick-card"><p class="quick-card-title">Sin cursos</p><p class="quick-card-subtitle">No hay cursos activos.</p></section>';return;}educationContent.innerHTML=courses.map(course=>'<section class="quick-card mt-1"><p class="quick-card-title">'+course.title+'</p><p class="quick-card-subtitle">'+(course.description||'')+'</p>'+course.lessons.map(lesson=>'<article class="member-card mt-1"><div><p class="member-name">'+lesson.title+'</p><div class="member-meta">'+lesson.parts.map(renderPart).join('')+'</div></div></article>').join('')+'</section>').join('');}catch(error){if(error.status===401){clearToken();window.location.href='/login';return;}educationContent.innerHTML='<section class="quick-card"><p class="quick-card-title">Error</p><p class="quick-card-subtitle">No se pudo cargar educacion.</p></section>';}}
