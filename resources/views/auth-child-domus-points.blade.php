...
<main class="layout">
    ...
    <section class="page content card">
        ...
        <section class="mt-2">
            <p class="quick-card-title">Mis misiones Domus</p>
            <div id="missions-list" class="education-list mt-1"></div>
        </section>
...
function renderMissions(items){if(!items.length){missionsList.innerHTML='<section class="quick-card"><p class="quick-card-subtitle">Todavia no hay misiones disponibles.</p></section>';return;}missionsList.innerHTML=items.map(mission=>'<article class="education-card"><p class="education-card-title">'+escapeHtml(mission.title)+'</p><p class="education-card-text">'+escapeHtml(mission.description||'')+'</p><div class="lesson-status-row"><span class="progress-pill">+'+Number(mission.points_reward||0)+' puntos</span><span class="progress-pill">'+(mission.is_completed?'Completada':'Pendiente')+'</span></div></article>').join('');}
