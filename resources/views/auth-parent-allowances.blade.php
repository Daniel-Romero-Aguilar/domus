...
<head>
    ...
    <style>
        ...
        .allowance-alert-overdue{border-left-color:#0b5cad;background:#eaf3ff;color:#082b60;}
        .allowance-alert small{display:block;margin-top:.2rem;font-weight:600;opacity:.9;}
        .allowance-system-alert{margin-top:1rem;padding:.9rem 1rem;border:1px solid #bad7ff;border-left:5px solid #0b5cad;background:#eef6ff;color:#082b60;border-radius:.7rem;font-size:.94rem;font-weight:700;line-height:1.4;}
        .allowance-system-alert.is-error{border-color:#ffd2d2;border-left-color:#b3261e;background:#fff1f1;color:#731817;}
        .allowance-system-alert small{display:block;margin-top:.25rem;font-weight:600;}
...
<main class="layout">
    ...
    <section class="page content card">
        ...
        <div id="allowances-system-alert" class="allowance-system-alert is-hidden"></div>
...
<template id="template-allowance-details">
    <section id="allowance-detail-panel" class="quick-card allowance-detail-card mt-1 is-hidden">
        ...
        <div class="allowance-detail-grid mt-1">
            ...
            <div><p class="allowance-detail-label">Pagos pendientes</p><p data-field="due_payments" class="allowance-detail-value"></p></div>
            <div><p class="allowance-detail-label">Ultimo problema</p><p data-field="last_problem" class="allowance-detail-value"></p></div>
        </div>
    </section>
</template>
...
function getDuePaymentLabel(allowance){const count=Number(allowance.due_payments_count||0);if(count<1){return '0';}return count+(allowance.due_payments_count_capped?'+':'');}
function setSystemAlert(message,type){allowancesSystemAlert.innerHTML=message;allowancesSystemAlert.className='allowance-system-alert '+(type==='error'?'is-error':'');}
function clearSystemAlert(){allowancesSystemAlert.textContent='';allowancesSystemAlert.className='allowance-system-alert is-hidden';}
function renderSystemAlert(summary){const pending=Number(summary?.pending_due_payments||0),paused=Number(summary?.paused_due_allowances||0),pendingLabel=pending+(summary?.pending_due_payments_capped?'+':'');if(pending>0){setSystemAlert('Pagos vencidos pendientes: '+pendingLabel+'.<small>El proceso de mesadas va atrasado o estuvo detenido. Cuando vuelva a correr, los pagara en orden y sin duplicarlos.</small>','info');return;}if(paused>0){setSystemAlert('Hay '+paused+' mesada'+(paused===1?'':'s')+' pausada'+(paused===1?'':'s')+' por pagos fallidos.<small>Revisa las tarjetas marcadas para ver el motivo.</small>','info');return;}clearSystemAlert();}
function getAllowanceAlert(allowance){const dueCount=Number(allowance.due_payments_count||0),dueLabel=getDuePaymentLabel(allowance),reason=String(getLatestFailureReason(allowance)||'').toLowerCase();if(allowance.status!=='paused'&&dueCount>0){return '<div class="allowance-alert allowance-alert-overdue">Pagos pendientes del servidor: '+escapeHtml(dueLabel)+'.<small>Se pondran al corriente cuando el proceso de mesadas vuelva a correr.</small></div>';}if(allowance.status!=='paused'){return '';}if(reason.includes('fondos')||reason.includes('saldo')){return '<div class="allowance-alert">Mesada pausada por una regla anterior de saldo.<small>Intenta ejecutarla de nuevo; el padre ya no necesita saldo disponible.</small></div>';}return '<div class="allowance-alert">Mesada pausada.<small>Revisa el detalle para ver el motivo.</small></div>';}
...
function updateBalanceNote(){parentBalanceNote.textContent='Dinero entregado o comprometido registrado: '+toMoneyFromCents(currentParentBalance);}
