...
loanForm.addEventListener('submit',async event=>{event.preventDefault();clearFeedback();const interestPayload=getInterestPayload();const payload={child_user_id:currentUserId,amount:loanAmount.value.trim(),reason:loanReason.value.trim()||null,due_date:loanDueDate.value,installments_count:getInstallmentsCount(),installment_frequency:frequency.value,has_interest:interestPayload.has_interest,interest_mode:interestPayload.interest_mode,annual_interest_rate:interestPayload.annual_interest_rate,fixed_interest_amount:interestPayload.fixed_interest_amount};let response;try{response=await apiRequest('/loans','POST',payload);}catch(error){setFeedback(error?.data?.message||'No se pudo crear la solicitud.','feedback-error');return;}loanForm.reset();resetLoanSelections();deferredPaymentFields.classList.add('is-hidden');interestModeRow.classList.add('is-hidden');interestOptions.classList.add('is-hidden');customInterest.classList.add('is-hidden');setFeedback(response?.message||'Solicitud enviada al administrador.','feedback-success');await loadLoansList();});
