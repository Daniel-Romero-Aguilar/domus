...
<head>
    ...
    <style>
        ...
        .loan-payments-table-wrap{overflow-x:auto;border:1px solid #d8e0f2;border-radius:.85rem;background:#fff;}
        .loan-payments-table{width:100%;border-collapse:collapse;min-width:620px;}
        .loan-payments-table th,.loan-payments-table td{padding:.75rem .85rem;border-bottom:1px solid #e8edf7;text-align:left;font-size:.92rem;}
        .loan-payments-table th{color:#6b7a99;font-size:.8rem;text-transform:uppercase;letter-spacing:.04em;}
        .loan-payments-table tr:last-child td{border-bottom:0;}
...
<template id="template-loan-details">
    <section id="loan-detail-panel" class="quick-card loan-detail-card mt-1 is-hidden">
        ...
        <div class="loan-detail-grid mt-1">
            ...
            <div><p class="loan-detail-label" data-field="date_label">Fecha limite</p><p data-field="due_date" class="loan-detail-value"></p></div>
        ...
        <section class="mt-2">
            ...
            <div class="loan-payments-table-wrap mt-1">
                <table class="loan-payments-table">
                    <thead>
                        <tr>
                            <th>Cuota</th>
                            <th>Fecha</th>
                    ...
                    <tbody data-field="payments-rows"></tbody>
                </table>
            </div>
        </section>
    </section>
...
function formatDate(value){if(!value){return '-';}const date=new Date(value+'T12:00:00');if(Number.isNaN(date.getTime())){return String(value);}return date.toLocaleDateString('es-MX');}
...
function renderPaymentRows(payments,loanStatus){if(!Array.isArray(payments)||payments.length===0){return '<tr><td colspan="8">Este prestamo aun no tiene pagos programados.</td></tr>';}return payments.map(payment=>'<tr><td>#'+Number(payment.installment_number||0)+'</td><td>'+escapeHtml(formatDate(payment.due_date))+'</td><td>'+getStatusChip(payment.status,payment.status_label)+'</td><td>'+escapeHtml(payment.total_amount_display||toMoney(payment.total_amount))+'</td><td>'+escapeHtml(payment.principal_amount_display||toMoney(payment.principal_amount))+'</td><td>'+escapeHtml(payment.interest_amount_display||toMoney(payment.interest_amount))+'</td><td>'+escapeHtml(payment.paid_at?formatDateTime(payment.paid_at):'-')+'</td><td>'+(loanStatus==='approved'&&payment.is_payable_today&&payment.status!=='paid'?'<button class="btn gold-btn btn-inline pay-loan-btn" type="button" data-loan-payment-id="'+Number(payment.id)+'">Pagar</button>':'-')+'</td></tr>').join('');}
