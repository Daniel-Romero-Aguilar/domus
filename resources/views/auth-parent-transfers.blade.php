...
let currentParentBalance=0,transferTargets=[],selectedMemberId=null,currentTransferDetailPanel=null,transfersById=new Map(),transferIdempotencyKey=generateIdempotencyKey();
...
const transferAmount=document.getElementById('transfer-amount'),transferIdempotencyKey=document.getElementById('transfer-idempotency-key');
...
function openTransferForm(target){selectedMemberId=target.user_id;selectedMemberLabel.textContent=target.name+' (@'+(target.username||'sin_username')+')';transferIdempotencyKey.value=generateIdempotencyKey();membersPanel.classList.add('is-hidden');formPanel.classList.remove('is-hidden');clearFeedback();}
...
async function submitTransfer(){const amountCents=parseMoneyToCents(transferAmount.value);if(amountCents===null||amountCents<1){setFeedback('Escribe un monto valido con hasta 2 decimales.','feedback-error');return;}if(!selectedMemberId){setFeedback('Selecciona un miembro antes de enviar dinero.','feedback-error');return;}transferSubmit.disabled=true;try{const response=await apiRequest('/transfers','POST',{child_user_id:selectedMemberId,amount:transferAmount.value.trim()},{'Idempotency-Key':transferIdempotencyKey.value});if(Number.isFinite(Number(response.remaining_parent_balance))){currentParentBalance=Number(response.remaining_parent_balance);updateBalanceNote();}setFeedback(response.message||'Dinero enviado correctamente.',response.executed===false?'feedback-error':'feedback-success');transferAmount.value='';transferIdempotencyKey.value=generateIdempotencyKey();await loadTransfers();}catch(error){setFeedback(error?.data?.message||'No se pudo enviar el dinero.','feedback-error');transferIdempotencyKey.value=generateIdempotencyKey();}finally{transferSubmit.disabled=false;}}
