...
function buildLoanPayload(){const interestPayload=getInterestPayload();return {child_user_id:selectedMemberId,amount:loanAmount.value.trim(),reason:loanReason.value.trim()||null,due_date:loanDueDate.value,installments_count:getInstallmentsCount(),installment_frequency:frequency.value,has_interest:interestPayload.has_interest,interest_mode:interestPayload.interest_mode,annual_interest_rate:interestPayload.annual_interest_rate,fixed_interest_amount:interestPayload.fixed_interest_amount};}
...
loanConfirmAccept.addEventListener('click',async()=>{if(!pendingLoanPayload){return;}loanConfirmAccept.disabled=true;let response;try{response=await apiRequest('/loans','POST',pendingLoanPayload);}catch(error){loanConfirmPanel.classList.add('is-hidden');formPanel.classList.remove('is-hidden');setFeedback(error?.data?.message||'No se pudo crear el prestamo.','feedback-error');loanConfirmAccept.disabled=false;return;}if(Number.isFinite(Number(response.remaining_balance))){currentParentBalance=Number(response.remaining_balance);updateBalanceNote();}resetLoanFormUI();await loadLoansList();showList();setFeedback(response?.message||'Prestamo ofrecido correctamente.','feedback-success');loanConfirmAccept.disabled=false;});
