...
function updateBalanceNote(){parentBalanceNote.textContent='* Cuentas con '+toMoney(currentParentBalance)+' para dar prestamos';}
...
(async function bootstrap(){updateScrollHint();if(!getToken()){window.location.href='/login';return;}try{const me=await apiRequest('/me','GET');if(me.user?.role!=='parent'){window.location.href='/member/loans';return;}sidebarUserName.textContent=me.user?.name||'Usuario';currentParentBalance=Number(me.user?.balance?.amount||0);updateBalanceNote();await fetchFamilyMembers();await loadLoansList();showList();}catch(error){if(error.status===401){clearToken();window.location.href='/login';return;}loansList.innerHTML='<section class="quick-card"><p class="quick-card-title">Error de sesion</p><p class="quick-card-subtitle">No se pudo validar la sesion.</p></section>';}})();
</script>
</body>
</html>
