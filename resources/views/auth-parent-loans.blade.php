...
<head>
    ...
    <style>
        ...
        .loan-payments-table{width:100%;border-collapse:collapse;min-width:580px;}
...
function renderPaymentRows(payments){if(!Array.isArray(payments)||payments.length===0){return '<tr><td colspan="7">Este prestamo aun no tiene pagos programados.</td></tr>';}return payments.map(payment=>'<tr><td>#'+Number(payment.installment_number||0)+'</td><td>'+escapeHtml(formatDate(payment.due_date))+'</td><td>'+getStatusChip(payment.status,payment.status_label)+'</td><td>'+escapeHtml(payment.total_amount_display||toMoney(payment.total_amount))+'</td><td>'+escapeHtml(payment.principal_amount_display||toMoney(payment.principal_amount))+'</td><td>'+escapeHtml(payment.interest_amount_display||toMoney(payment.interest_amount))+'</td><td>'+escapeHtml(payment.paid_at?formatDateTime(payment.paid_at):'-')+'</td></tr>').join('');}
