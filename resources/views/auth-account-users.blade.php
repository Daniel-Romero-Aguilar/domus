...
async function loadUserHeader(){if(!getToken()){window.location.href='/login';return;} try{const data=await apiRequest('/me','GET'); sidebarUserName.textContent=data.user?.name||'Usuario';if(tasksLink){tasksLink.href=(data.user?.role==='parent'?'/parent/tasks':'/child/tasks');}if(transfersLink){transfersLink.href='/parent/transfers';transfersLink.classList.toggle('is-hidden',data.user?.role!=='parent');}if(allowancesLink){allowancesLink.href='/parent/allowances';allowancesLink.classList.toggle('is-hidden',data.user?.role!=='parent');}if(savingsBoxesLink){savingsBoxesLink.href='/parent/savings-boxes';savingsBoxesLink.classList.toggle('is-hidden',data.user?.role!=='parent');}if(loansLink){loansLink.href=(data.user?.role==='parent'?'/parent/loans':'/member/loans');}}catch(error){if(error.status===401){clearToken();window.location.href='/login';}}}
