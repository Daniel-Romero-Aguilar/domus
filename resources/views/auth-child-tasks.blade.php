...
async function markTaskCompletedByMember(taskId){try{const response=await apiRequest('/tasks/member/completed/'+taskId,'POST');if(window.handleDomusBadges){window.handleDomusBadges(response.unlocked_badges);}showTaskFeedback('Muy bien, le avisaremos al admin que terminaste la tarea. Cuando el confirme, se te dara tu recompensa.','success');await loadTasks();}catch(error){showTaskFeedback(error?.data?.message||'No se pudo marcar la tarea como completada.','error');}}
