...
<main class="layout">
    ...
    <aside id="sidebar" class="sidebar">
        ...
        <div id="sidebar-scroll" class="sidebar-scroll">
            <nav class="sidebar-nav">
                ...
                <a class="sidebar-link is-active" href="/child/withdrawals"><span class="nav-icon">&bull;</span><span>Retirar dinero</span></a>
    ...
    <section class="page content card">
        ...
        <p class="subtitle">Pide retirar dinero fisico. Tu saldo se reserva hasta que tu padre acepte.</p>
...
function renderCard(item,canAccept){const actions=item.status==='pending_child'?'<div class="withdrawal-actions"><button class="btn gold-btn btn-inline" type="button" data-action="accept" data-id="'+Number(item.id)+'">Aceptar</button><button class="btn blue-btn btn-inline" type="button" data-action="cancel" data-id="'+Number(item.id)+'">Cancelar</button></div>':'';return '<section class="quick-card mt-1 withdrawal-card"><p class="quick-card-title">'+escapeHtml(item.amount_display||'$0.00')+'</p><p class="quick-card-subtitle">Estado: '+escapeHtml(item.status_label||item.status)+'</p><p class="withdrawal-muted">Inicio: '+escapeHtml(formatDate(item.created_at))+'</p>'+(canAccept?actions:'')+'</section>';}
...
withdrawalForm.addEventListener('submit',async event=>{event.preventDefault();clearFeedback();submitBtn.disabled=true;try{const response=await apiRequest('/child/withdrawals','POST',{amount:amountInput.value.trim()});showFeedback(response.message||'Retiro solicitado.','feedback-success');withdrawalForm.reset();await loadWithdrawals();}catch(error){showFeedback(error?.data?.message||'No se pudo solicitar el retiro.','feedback-error');}finally{submitBtn.disabled=false;}});
...
(async function bootstrap(){updateScrollHint();if(!getToken()){window.location.href='/login';return;}try{const me=await apiRequest('/me','GET');if(me.user?.role==='parent'){window.location.href='/parent/withdrawals';return;}if(me.user?.role!=='child'&&me.user?.role!=='member'){window.location.href='/account';return;}sidebarUserName.textContent=me.user?.name||'Usuario';await loadSidebarLevel();await loadWithdrawals();}catch(error){if(error.status===401){clearToken();window.location.href='/login';return;}withdrawalsList.innerHTML=renderEmpty('No se pudo validar la sesion.');}})();
</script>
</body>
</html>
